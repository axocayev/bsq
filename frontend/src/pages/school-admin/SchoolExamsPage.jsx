...
import {
  Table, Tag, Typography, Input, Button, Modal, Form, InputNumber,
  Space, message, DatePicker, Popconfirm,
} from 'antd';
import { EditOutlined, CheckOutlined, CloseOutlined, TeamOutlined, ClockCircleOutlined } from '@ant-design/icons';
import { useTranslation } from 'react-i18next';
import { useNavigate } from 'react-router-dom';
import dayjs from 'dayjs';
import {
  getSchoolExams, schoolAdminUpdateExam, schoolAdminChangeStartTime,
  schoolAdminApproveExam, schoolAdminRejectExam,
} from '../../api/exams';
...
export default function SchoolExamsPage() {
  ...
  const [startTimeModal, setStartTimeModal] = useState(null);
  ...
  const load = () => {
    setLoading(true);
    getSchoolExams({ size: 100 }).then(({ data }) => setExams(data.content)).finally(() => setLoading(false));
  };

  useEffect(() => { load(); }, []);

  const filtered = exams.filter((e) =>
    [e.title, e.createdByName, e.status].some((f) =>
  ...
  const onEdit = async () => {
    ...
    try {
      await schoolAdminUpdateExam(editModal.id, payload);
  ...
  const openStartTime = (exam) => {
    setStartTimeModal(exam);
    setNewStartDate(exam.startDate ? dayjs(exam.startDate) : null);
  };

  const onChangeStartTime = async () => {
    if (!newStartDate) return message.warning(t('exams.startDateRequired'));
    try {
      await schoolAdminChangeStartTime(startTimeModal.id, { startDate: newStartDate.format('YYYY-MM-DDTHH:mm:ss') });
      message.success(t('exams.startTimeChanged'));
      setStartTimeModal(null);
      load();
    } catch { message.error(t('common.failed')); }
  };

  const onApprove = async (exam) => {
    try { await schoolAdminApproveExam(exam.id); message.success(t('exams.approved')); load(); }
    catch { message.error(t('common.failed')); }
  };

  const onReject = async () => {
    try {
      await schoolAdminRejectExam(rejectModal.id, { reason: rejectReason });
    ...
    },
    {
      title: t('common.actions'), render: (_, r) => (
        <Space wrap>
          ...
          <Button size="small" icon={<ClockCircleOutlined />} onClick={() => openStartTime(r)}>{t('exams.changeStartTime')}</Button>
          <Button size="small" icon={<TeamOutlined />} onClick={() => navigate(`/school-admin/exams/${r.id}/assignments`)}>{t('exams.students')}</Button>
  ...
  return (
    <>
      ...
      <Modal title={t('exams.changeStartTime')} open={!!startTimeModal} onOk={onChangeStartTime} onCancel={() => setStartTimeModal(null)}>
        <Text type="secondary" style={{ display: 'block', marginBottom: 12 }}>{startTimeModal?.title}</Text>
        <DatePicker showTime format="YYYY-MM-DD HH:mm" style={{ width: '100%' }} value={newStartDate} onChange={(v) => setNewStartDate(v)} />
      </Modal>
