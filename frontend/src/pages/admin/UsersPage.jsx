...
import { Table, Button, Modal, Form, Input, Select, Space, Popconfirm, message, Tag, Typography, Row, Col } from 'antd';
import { PlusOutlined, EditOutlined } from '@ant-design/icons';
import { useTranslation } from 'react-i18next';
import { getUsers, createUser, deleteUser, adminUpdateUser } from '../../api/users';
import { getSchools } from '../../api/schools';

const { Title } = Typography;
const ROLES = ['ADMIN', 'SCHOOL_ADMIN', 'TEACHER', 'STUDENT'];
const roleColor = { ADMIN: 'red', SCHOOL_ADMIN: 'orange', TEACHER: 'blue', STUDENT: 'green' };
...
export default function UsersPage() {
  ...
  const role = Form.useWatch('role', createForm);
  const [search, setSearch] = useState('');
  const { t } = useTranslation();

  const filtered = users.filter((u) =>
    [u.username, u.fullName, u.email, u.role, u.schoolName].some((f) =>
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [u, s] = await Promise.all([getUsers({ size: 100 }), getSchools({ size: 100 })]);
  ...
  const onEdit = async () => {
    ...
    try {
      await adminUpdateUser(editModal.id, payload);
  ...
  const onDelete = async (id) => {
    try { await deleteUser(id); message.success(t('common.deleted')); load(); }
  ...
  const columns = [
    ...
    { title: t('users.school'), dataIndex: 'schoolName', render: (v) => v || '—' },
    {
      title: t('common.actions'), render: (_, r) => (
        <Space>
          <Button size="small" icon={<EditOutlined />} onClick={() => openEdit(r)}>{t('common.edit')}</Button>
          <Popconfirm title={t('users.deactivateConfirm')} onConfirm={() => onDelete(r.id)}>
  ...
  return (
    <>
      ...
      {/* Create modal */}
      <Modal title={t('users.createUser')} open={createOpen} onOk={onCreate} onCancel={() => setCreateOpen(false)}>
        <Form form={createForm} layout="vertical">
          ...
          {role && role !== 'ADMIN' && (
            <Form.Item name="schoolId" label={t('users.school')} rules={[{ required: true }]}>
      ...
      >
        <Form form={editForm} layout="vertical">
          ...
          <Form.Item name="phone" label={t('users.phone')}><Input /></Form.Item>
          <Form.Item name="password" label={t('users.newPasswordOptional')} extra={t('users.newPasswordOptional')}>
